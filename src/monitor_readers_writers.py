    ...
    def start_read(self, reader_id: int, color: str):
        ...
        with self.condition:
            ...
            while self.active_writer or self.writers_waiting > 0:
                print(f"{color}Lector {reader_id} esperando, hay escritor activo o en espera{RESET}")
                self.condition.wait()
    ...
    def start_write(self, writer_id: int, color: str):
        ...
        with self.condition:
            ...
            while self.readers_count > 0 or self.active_writer:
                print(f"{color}Escritor {writer_id} esperando, hay lectores activos o un escritor activo{RESET}")
                self.condition.wait()
...
class Reader(threading.Thread):
    ...
    def __init__(
        ...
        Args:
            reader_id (int): ID del lector.
            readers_writers (ReadersWriters): Instancia del monitor de lectores y escritores.
            color (str): Color del lector para imprimir mensajes en la consola.
            min_reading_time (int, optional): Tiempo mínimo de lectura. Defaults to MIN_READING_TIME.
            max_reading_time (int, optional): Tiempo máximo de lectura. Defaults to MAX_READING_TIME.
            min_thinking_time (int, optional): Tiempo mínimo entre lecturas. Defaults to 2.
            max_thinking_time (int, optional): Tiempo máximo entre lecturas. Defaults to 5.
        """
    ...
    def run(self):
        ...
        while True:
            ...
            print(f"{self.color}Lector {self.reader_id} pensando durante {think_time} segundos{RESET}")
            ...
            reading_time = random.randint(self.min_reading_time, self.max_reading_time)
            print(f"{self.color}Lector {self.reader_id} leyendo durante {reading_time} segundos{RESET}")
...
class Writer(threading.Thread):
    ...
    def __init__(
        ...
        Args:
            ...
            readers_writers (ReadersWriters): Instancia del monitor de lectores y escritores.
            color (str): Color del escritor para imprimir mensajes en la consola.
            min_writing_time (int, optional): Tiempo mínimo de escritura. Defaults to MIN_WRITING_TIME.
            max_writing_time (int, optional): Tiempo máximo de escritura. Defaults to MAX_WRITING_TIME.
            min_thinking_time (int, optional): Tiempo mínimo entre escrituras. Defaults to 3.
            max_thinking_time (int, optional): Tiempo máximo entre escrituras. Defaults to 8.
        """
        threading.Thread.__init__(self)
        self.writer_id = writer_id
        self.readers_writers = readers_writers
        self.color = color
        self.min_writing_time = min_writing_time
        self.max_writing_time = max_writing_time
        self.min_thinking_time = min_thinking_time
        self.max_thinking_time = max_thinking_time
        self.daemon = True
    ...
    def run(self):
        ...
        while True:
            ...
            think_time = random.randint(self.min_thinking_time, self.max_thinking_time)
            print(f"{self.color}Escritor {self.writer_id} pensando durante {think_time} segundos{RESET}")
            time.sleep(think_time)
            ...
            writing_time = random.randint(self.min_writing_time, self.max_writing_time)
            print(f"{self.color}Escritor {self.writer_id} escribiendo durante {writing_time} segundos{RESET}")
            time.sleep(writing_time)
            
            # Generar nuevo contenido
            new_content = f"Contenido escrito por Escritor {self.writer_id} en {time.time()}"
